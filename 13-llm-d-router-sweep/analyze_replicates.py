...
LEVELS = (96, 128)
PODS = 4
ARMS = (("epp-baseline", "llm-d default"), ("epp-thunder", "most-room"), ("epp-thunder-origin", "origin-only"))
RATIOS = (("epp-thunder", "epp-baseline", "most-room / default"), ("epp-thunder-origin", "epp-thunder", "origin / most-room"))
REPS = (1, 2, 3)
WARMUP_S = 600.0
SLO_S = 30.0
...
def main():
    ...
    rows = [("throughput (tok/s)", "throughput", "{:.0f}"), ("steady-state hit rate", "hit_steady", "{:.3f}"), ("TTFT p50 (s)", "ttft_p50", "{:.1f}"),
            ...
            ("EPP rebinds", "rebinds", "{:.0f}"), ("EPP origin waits", "origin_waits", "{:.0f}")]
    ...
    COL = {"epp-baseline": "#B64342", "epp-thunder": "#0F4D92", "epp-thunder-origin": "#3E9B4F"}
    panels = [("throughput", "Throughput", "output tokens / s"), ("hit_steady", "Prefix-cache hit rate", "steady state"), ("ttft_p50", "TTFT p50", "seconds"), ("attain_lenient", "Session SLO attainment", f"share of sessions, 95% of turns TTFT <= {SLO_S:.0f} s")]
    fig, axes = plt.subplots(1, 5, figsize=(21, 5), gridspec_kw={"width_ratios": [1, 1, 1, 1, 0.5]})
    ...
    for ax, (key, title, unit) in zip(axes, panels):
        for i, (arm, label) in enumerate(ARMS):
            ...
            for c in LEVELS:
                ...
                for v in vals:
                    ax.plot(x[LEVELS.index(c)] + (i - 1) * w, v, "o", color="black", markersize=3.5, zorder=4)
            ax.bar(x + (i - 1) * w, means, w, color=COL[arm], edgecolor="black", linewidth=1.5, label=label, zorder=2)
            ax.errorbar(x + (i - 1) * w, means, yerr=[lo, hi], fmt="none", ecolor="black", elinewidth=1.5, capsize=4, zorder=3)
