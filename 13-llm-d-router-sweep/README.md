...
- `analyze_wait_cost.py`: the wait-cost curve (cached prefix fraction at the next turn vs prefix idle age, per arm; `wait-cost.md`, `wait-cost.png`).
- `run-optionb.sh`: the Option B arms (`thunder-origin-u15`, `thunder-origin-u15-f25`, thunder-agent-v5) at c=128.
...
Run `results/sweep-20260918-134248-t1900`: six levels on 2026-09-18 (13:42 to 23:35 PDT), three lower levels c = 16, 32, 64 appended on 2026-09-19 (10:24 to 15:15) to locate the throughput peak, then the `epp-thunder-origin` arm at c = 96 to 338 plus a v4 control cell (2026-09-19 15:37 to 2026-09-20 00:05), then two more replicates per arm at c = 96 and 128 (2026-09-20 01:00 to 08:35) one llm-d default cell per level with the session-id bench image (09:39 to 11:15), three 90-minute cells at c=128 (15:25 to 21:34), and the Option B cells on 2026-09-21 (five valid). 42 valid cells, no preemption; every cell of the first two arms had 0 request errors, two origin cells had 2 each (`400 - Context Window`). Full tables in `sweep.md` (including a `sessions completed and replaced` row), curves in `sweep.png` (log-scale x axis).
...
**Option B, first attempt (2026-09-21 02:53 to 08:00): the urgent tier at 15 s, with and without a 25 s forced-admission backstop.** llm-d-router commit `1e922b63` (tag `thunder-agent-v5`) adds `urgentWaitMs`: a paused or new head that has waited that long is ordered ahead of every non-urgent head (oldest first) and, under origin-only, may take any pod with room. Arms `thunder-origin-u15` (origin-only + `urgentWaitMs: 15000`) and `thunder-origin-u15-f25` (plus `headWaitStarvationMs: 25000`), c=128, two 30-minute cells each plus one 90-minute u15 cell; driver `run-optionb.sh`. The 90-minute u15-f25 cell did not run: the laptop lost its gcloud token and DNS for a few minutes at 08:08, which also made the driver misread an empty pod list as a preemption of the u15 90-minute cell (collected by hand, manifest note; `run-pool.sh` now ignores an empty pod list). Pre-registered criteria and outcome at 32 sessions per pod, 30-minute cells:

| metric | most-room (n=3) | origin-only (n=3) | origin + urgent 15 s (n=2) | + forced 25 s (n=2) | criterion for u15 |
|---|---|---|---|---|---|
| throughput (tok/s) | 1370 | 1693 | **1300** (1297-1303) | 1207 | >= 1510: failed |
| steady-state hit rate | 0.35 | 0.68 | 0.27 | 0.12 | |
| TTFT p50 / p90 (s) | 2.4 / 10.1 | 1.0 / 11.4 | 3.3 / **29.8** | 4.5 / 34.9 | |
| share of turns with TTFT > 30 s | 3.3% | 3.7% | **21.5%** | 37.9% | <= 3.3%: failed |
| session SLO attainment, strict | 0.76 | 0.70 | **0.33** | 0.12 | >= 0.76: failed |
| per-session worst TTFT, p90 (s) | 167 | 261 | 77 | 57 | <= 170: met |
| sessions whose worst turn exceeded 60 s | 14% | 21% | 21% | 6% | <= 14%: failed |
| rebinds / urgent promotions / forced admissions | 1091 / 0 / 0 | 0 / 0 / 0 | 586 / 804 / 0 | 384 / 550 / 510 | rebinds between: met |

The tier cut the extreme tail (worst-turn p90 from 261 to 77 s) and broke everything else. The TTFT distribution of turns shows why: under origin-only 72 percent of turns start within 5 s and only 8 percent wait more than 15 s; under u15 only 30 percent start within 5 s, 29 percent land in 15 to 30 s and 18 percent in 30 to 60 s. The 804 urgent promotions per cell are more than half of all turns, so the tier did not resolve pre-existing long waits, it created them. Mechanism: a head that reaches 15 s is moved to the pod with the most room with a full re-prefill of 60k to 80k tokens; that pod overfills, the sweep pauses its idle programs, their next turns wait, reach 15 s, and move in turn. Moves cost cache residency (hit rate 0.68 to 0.27), oldest-first sends the largest programs first, and every moved turn carries 15 s of wait plus a re-prefill, so it lands past 30 s. most-room avoids the loop by moving at once (no wait) and smallest first; origin-only avoids it by never moving. "Wait, then move" has both costs and neither benefit. The 90-minute u15 cell shows the loop deepening with session depth: in the 30 to 60 minute slice TTFT p50 27 s, hit rate 0.19, strict attainment 0.09, throughput 0.78x of origin-only. The 25 s backstop on top (510 forced admissions per cell) over-admits into full pods and lands near the llm-d default (attainment 0.12, hit rate 0.12).

What this settles: bounded origin wait with a move, at a threshold below the typical origin wait, is unstable on a saturated pool. What it leaves open, to test next as separate switches: age-priority without a move (keeps residency; helps only when the origin frees a large enough block), age-priority plus reserving the origin pod for the urgent head (stops smaller newcomers from taking each freed block; costs some idle room), and a threshold well above the typical wait (60 s) that touches only the 2 percent tail.

**Age-only (2026-09-21 09:56, one 30-minute cell, thunder-agent-v6, `urgentWaitMs: 15000`, `urgentMove: false`).** Reordering alone also fails: throughput 1461 tok/s (origin-only 1693), hit rate 0.51 with zero rebinds, 18 percent of turns over 30 s (3.7), strict attainment 0.39 (0.70), 34 percent of sessions with a worst turn over 60 s (21), 606 urgent promotions. Every prompt-size quartile got slower, including the largest (share of turns waiting over 15 s: 21 to 43 percent; TTFT p90 30 to 111 s). Oldest-first is largest-first: each freed block of room resumes fewer sessions, the queue lengthens for everyone (same pool occupancy: 68 running requests, KV 0.54 in both arms, but 1714 turns instead of 2090), and longer waits cost cache residency, which is the mechanism the next paragraph measures.

**The wait-cost curve (analysis of existing cells, `analyze_wait_cost.py`, `wait-cost.md`, `wait-cost.png`).** vLLM reports `cached_tokens` per request, so for every turn after warm-up we know how much of its prompt was still cached when it was prefilled, against the prefix idle age (previous turn's end to this turn's first token). At c=128:

| idle age | origin-only, median cached fraction | most-room | age-only | u15 (move) | share of origin-only turns |
|---|---|---|---|---|---|
| 0 to 2 s | 0.99 | 0.97 | 0.98 | 0.96 | 39% |
| 2 to 5 s | 0.98 | 0.08 | 0.97 | 0.95 | 22% |
| 5 to 10 s | 0.92 | 0.00 | 0.85 | 0.67 | 17% |
| 10 to 15 s | 0.17 | 0.00 | 0.13 | 0.00 | 9% |
...
**Wait cap 8 s (2026-09-21, three 30-minute cells at c=128, thunder-agent-v7, `resumePlacement: origin-only`, `originWaitMaxMs: 8000`, ordering unchanged; proposal Part 8 layer 1).** Cells show mean (min-max).

| metric | most-room (n=3) | origin-only (n=3) | urgent 15 s (n=2) | age-only 15 s (n=1) | **wait cap 8 s (n=3)** |
|---|---|---|---|---|---|
| throughput (tok/s) | 1370 (1362-1382) | 1693 (1571-1807) | 1300 | 1461 | **1484 (1456-1504)** |
| steady-state hit rate | 0.35 (0.34-0.35) | 0.68 (0.63-0.72) | 0.27 | 0.51 | 0.51 (0.50-0.53) |
| TTFT p50 / p90 (s) | 2.4 / 10.1 | 1.0 / 11.4 | 3.3 / 29.8 | 1.5 / 31.3 | 2.1 / 13.4 |
| share of turns with TTFT > 30 s | 3.3% (3.0-3.6) | 3.7% (3.6-3.7) | 21.5% | 18.3% | 3.7% (3.3-4.3) |
| strict / lenient session attainment | 0.76 (0.75-0.77) / 0.78 | 0.70 (0.69-0.72) / 0.75 | 0.33 / 0.34 | 0.39 / 0.41 | **0.77 (0.75-0.78) / 0.78** |
| per-session worst TTFT, p90 (s) | 167 (167-168) | 261 (219-302) | 77 | 171 | 183 (125-232) |
| sessions whose worst turn exceeded 60 s | 14% (13-14) | 21% (21-21) | 21% | 34% | **14% (14-15)** |
| goodput within SLO (turns/s) | 1.28 (1.27-1.29) | 1.53 (1.53-1.53) | 0.90 | 1.06 | 1.41 (1.36-1.44) |
| rebinds / origin waits / origin-wait moves / forced | 1091 / 0 / 0 / 0 | 0 / 1105 / 0 / 0 | 586 / 1280 / - / 0 | 0 / 1117 / 0 / 0 | 354 (351-358) / 1280 / 327 / 0 |

Against the Part 8 layer 1 predictions: throughput between the two arms, met (1484, although nearer most-room than predicted); rebinds about 20 percent of resumes, met (354 of about 1900, 18 to 19 percent); strict attainment at or above most-room's 0.76, met as a tie (0.77 vs 0.76, ranges overlap); worst-turn p90 at or below most-room's 167 s, not met on the mean (183 s, and this metric is noisy: 125 to 232 s across the three cells against most-room's tight 167 to 168).

Reading with error bars: the cap gives most-room's tail (same share of slow turns and of sessions with a worst turn over 60 s, same session attainment, all within cell noise) with 8 percent more throughput and 0.17 more hit rate than most-room; it gives up 12 percent throughput and 0.17 hit rate against origin-only in exchange for the shorter tail (worst-over-60 from 21 to 14 percent, strict attainment from 0.70 to 0.77). The single-cell reading that it beat most-room on every metric does not survive replication for the tail metrics, which tie; the throughput and hit-rate gains do. So the pool now has three operating points that are not dominated: origin-only for total work, most-room and the 8 s cap for the tail, with the cap strictly better than most-room among the two. The mechanism is visible in the wait-cost table (moves start at 8 s: the 5 to 10 s bin's cached fraction drops from 0.92 to about 0.76, the 10 to 15 s bin holds the moved, cold turns) and in the counters (327 moves per cell, 18 percent of resumes, no forced admissions). Next: Part 8 layer 2 (residency from the KV index) to make the cap adaptive and to stop moving the sessions whose prefix would have survived.
