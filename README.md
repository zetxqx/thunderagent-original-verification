...
`proposal/ADMISSION-ORDERING.md` - the question underneath Parts 1 and 3: when capacity frees, which session gets it? The port orders by smallest footprint, and on this corpus footprint is turn depth (27k tokens at turn 0-1, 90k at turn 20+), so "smallest first" is "newest first" in disguise. Little's law argues the opposite: admitting a newcomer adds a working set that grows, finishing a veteran releases one permanently, and step 13 shows the pool completes a fixed number of turns whichever way you split them (46 turns each across 81 sessions at c=48, 10 each across 338 at c=338). Origin-only resume already proved the same principle on the placement axis (1.20x to 1.24x throughput, hit rate up to 1.96x); the ordering axis is untested. Proposes ordering on residency value (recency is already stored on every program, prefix residency needs KV events), and notes that the fairness backstop such a change would need already exists as the `urgentWaitMs` tier. One paired arm at c=128 against the full corpus, the first run that would actually have newcomers competing with veterans.

**Part 8, residency-aware hold-or-move**: the wait-cost curve shows a paused session's prefix survives about 10 to 12 s on its pod at 32 sessions per pod, then is gone; origin-only's tail is sessions waiting for an origin that has already evicted them, most-room's cost is moving sessions whose prefix was still warm, and both 15 s policies acted on cold sessions. Proposal: decide hold-or-move from measured residency (llm-d's KV block index from vLLM KV events) and real free room, with a static 8 s origin-wait cap as the cheap first test.
