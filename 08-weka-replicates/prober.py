...
3. programs-timeline.csv - ThunderAgent /programs: one row per program per
   ...
   lives in `state`), backend, plus step_count and total_tokens.
   step_count is what makes a fair cross-arm comparison possible: the
   per-request report carries no session identity, so this is the only way
   to ask "for the sessions BOTH arms touched, who got further?".
Router and vLLM CPU usage is sampled separately by the driver via
`kubectl top pod` (the prober runs in the bench pod and cannot read another
pod's cgroup); see cpu-usage.csv next to these files.
...
PROG_FIELDS = ["ts", "program_id", "status", "state", "backend",
               "step_count", "total_tokens", "context_len"]
        ...
        try:
            ...
            for pid, st in progs.items():
                prog_csv.row({"ts": ts, "program_id": pid, "status": st.get("status"),
                              ...
                              "context_len": st.get("context_len")})
        except Exception as e:
