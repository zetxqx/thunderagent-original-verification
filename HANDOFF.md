...
| inference-perf with the permit fix and the per-request session id | `~/projects/llmdthunder/inference-perf/`, branch `fix-session-replay-permits`, commits `d5a7c8c` and `d2bfa20`, PR https://github.com/zetxqx/inference-perf/pull/3; bench image `inference-perf:session-id-v1` (digest `f7381b01`, in `12-llm-d-router-pool/results/inference-perf-image.txt`), previous image recorded next to it |
...
| images | EPP `us-central1-docker.pkg.dev/bobzetian-gke-dev/bobinference/llm-d-router-endpoint-picker:thunder-agent-v3` (= commit `ae371354`), `:thunder-agent-v4` (= `8ee881c2`, adds `resumePlacement`; identical under the default), `:thunder-agent-v5` (= `f6ee4130`, adds `urgentWaitMs`; to be built). Each commit is pinned by an annotated tag of the same name on `zetxqx/llm-d-router`, so a rebase of the branch cannot orphan it; the full table is in `13-llm-d-router-sweep/README.md`, section "Code versions behind the cells"; load generator `.../inference-perf@sha256:63f2fbcf...` (permit fix), also in `12-llm-d-router-pool/results/inference-perf-image.txt` |

## Cluster state right now

- Helm release `program-aware-scheduling` (the main EPP, 1 replica, envoy sidecar, service port 80 -> envoy 8081, metrics 9090): revision 62, after the step 13 long-window cells (2026-09-20 21:34): image **`thunder-agent-v4`**, **thunder** (most-room) plugin config and envoy `message_timeout` 2400 s, left by the last cell `epp-thunder-c128-w90`; revision 22 was the step 09 state (thunder config, 1000 s). Whatever arm ran last leaves its config on the release. `12-llm-d-router-pool/run-pool.sh` upgrades it per cell with `main-values.yaml` (2400 s timeout) and reverts are documented in `08-cluster-changes.md`.
...
- Pool cells take about 45 to 50 minutes end to end for a 30-minute window and about 120 to 128 minutes for a 90-minute window (the drain after the stage plus report collection is 20 to 35 minutes).
- The bench pod's 700 MB corpus download from HuggingFace is sometimes truncated (twice in 40 cells: 10 traces once, 0 traces once); `run-pool.sh` now voids such cells (`-SHORTCORPUS` suffix) and they are rerun.
- The generator starts all sessions at t=0 and starts a new trace whenever one ends. Under heavy load sessions rarely finish (0 to 25 of 128 to 338 slots in 30 min), but under light load they do: at c=48 about two thirds of the slots turned over in 30 min (30 to 33 completed sessions, median 9 min). Low-concurrency cells therefore mix in trace turnover (cold starts, early short-prompt turns); see the `sessions completed and replaced` row in the step 13 tables. The filtered corpus has exactly 338 traces, so at c=338 finished sessions are NOT replaced and concurrency decays by up to 6 percent within the cell; run replicates at c <= 256. The faster arm finishes more sessions and pulls in more cold traces, which makes the reported gains slightly conservative.

## Open items, in the order they were agreed

1. Step 13 sweep: DONE, including the c = 16, 32, 64 extension, the `epp-thunder-origin` arm at c = 96 to 338, and three-replicate comparisons of origin-only vs most-room at c = 96 and 128 (2026-09-20). Origin-only: throughput 1.20x [1.18-1.20] and 1.24x [1.14-1.32] over most-room, hit rate 0.82 and 0.68, rebinds 0. Session-level (Part 7, from the session-id cells, 30 s TTFT SLO, population = sessions active after warm-up): strict session attainment at 24 / 32 per pod is 0.58 / 0.08 for the llm-d default, 0.87 / 0.76 for most-room, 0.82 / 0.70 for origin-only; goodput within SLO 1.22 / 0.57, 1.09 / 1.28, 1.58 / 1.53 turns/s. The default fails the session SLO before its throughput falls; origin-only wins goodput and typical progress and loses 5 to 6 attainment points. 90-minute cells at c=128 (one per arm, 30-minute slices): the port's gain over the default grows with session depth (1.26x, 1.45x, 1.20x; the default's hit rate hits 0.002 and TTFT p50 41 s in the deepest slice), origin-only's extra gain over most-room shrinks with depth (1.24x, 1.06x, 1.11x) while its attainment penalty grows (0.78x, 0.64x, 0.73x). Next change: Part 3 Option B (bounded origin wait), judged by session SLO attainment in the deep regime. Not yet committed in this repo: the replicates, SATURATION.md, Part 7, the inference-perf session-id image record.
